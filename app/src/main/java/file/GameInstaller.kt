...
class GameInstaller(path: String) {
    ...
    fun check(): Boolean {
        try {
            ...
            val iniFile = findCaseInsensitive(INI_NAME)
            val dataFilesDir = findCaseInsensitive(DATA_NAME)
            return iniFile != null && dataFilesDir != null
        } catch (e: Exception) {
            e.printStackTrace()
            return false
        }
    ...
    private fun findCaseInsensitive(name: String): File? {
        ...
        return dir
            .listFiles()
            ?.firstOrNull { it.name.toLowerCase() == nameLower }
    }
    ...
    companion object {
        private const val INI_NAME = "Morrowind.ini"
        private const val DATA_NAME = "Data Files"
